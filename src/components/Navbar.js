// Navbar renders the navbar UI component.
import React, { useState, useEffect } from 'react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { useTheme } from '../context/ThemeContext';
import { FaSearch, FaUser, FaChartBar, FaSignOutAlt, FaHome, FaDatabase, FaBars, FaTimes, FaSun, FaMoon, FaProjectDiagram } from 'react-icons/fa';
const Navbar = () => {
  const {
    user,
    logout
  ...
  const navBg = isDark ? scrolled ? 'rgba(10,15,30,0.95)' : 'rgba(10,15,30,0.78)' : scrolled ? 'rgba(255,255,255,0.97)' : 'rgba(255,255,255,0.88)';
  ...
  return <nav style={{
    ...
    boxShadow: scrolled ? `0 4px 30px rgba(0,0,0,0.3), 0 0 40px rgba(6,182,212,0.05)` : 'none',
    ...
    zIndex: 50
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {}
          <Link to="/dashboard" className="flex items-center gap-2 group" style={{
          textDecoration: 'none'
        }}>
            <div style={{
            width: 36,
          ...
          <div className="flex items-center gap-2">
            ...
            {}
            {user && <button className="sm:hidden" onClick={() => setMobileOpen(!mobileOpen)} style={{
            color: textMuted,
            background: 'none',
            border: 'none',
            cursor: 'pointer'
        ...
        {}
        {user && mobileOpen && <div className="sm:hidden pb-4 animate-fade-in" style={{
