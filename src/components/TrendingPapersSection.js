...
const TrendingPapersSection = ({ isDark, t }) => {
  ...
  const load = useCallback(async (year) => {
    setLoading(true);
    try {
      const data = await statsApi.getTrendingPapers(year, 6, { curated: true });
    ...
    <div ref={ref} className="animate-fade-in" style={{ marginBottom: '2.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.72rem', color: t.textMuted, margin: 0 }}>
              Collaboration-first preview with extreme outliers grouped lower
            </p>
          </div>
        </div>
      ...
      {meta?.extreme_in_sample > 0 && (
        <div style={{
          ...
          background: accentFireBg, border: `1px solid ${accentFireBorder}`,
          borderRadius: 12, color: accentFire,
        }}>
          <FaLayerGroup size={12} />
          <span style={{ fontSize: '0.78rem', fontWeight: 600 }}>
            Dashboard preview prioritises {meta.preferred_author_range}. Consortium-scale papers are still available in the full catalog.
          </span>
        </div>
      )}
        ...
        <div style={{
          ...
          {papers.map((paper, i) => (
            <TrendingCard
              ...
              rank={i + 1}
              t={t}
              accentFire={accentFire}
              accentFireBg={accentFireBg}
              accentFireBorder={accentFireBorder}
            />
          ))}
        </div>
      )}
...
const TrendingCard = ({ paper, rank, t, accentFire, accentFireBg, accentFireBorder }) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? '0 8px 28px rgba(0,0,0,0.3),0 0 16px rgba(245,158,11,0.12)' : '0 2px 8px rgba(0,0,0,0.2)',
      ...
      <div style={{ paddingLeft: '0.4rem' }}>
        <div style={{
          ...
          <div style={{
            ...
            background: accentFireBg, border: `1px solid ${accentFireBorder}`,
            borderRadius: 999, padding: '0.15rem 0.6rem', flexShrink: 0,
          }}>
            <FaUsers size={10} style={{ color: accentFire }} />
            <span style={{ fontSize: '0.72rem', fontWeight: 700, color: accentFire }}>
              {paper.author_count_display || `${paper.author_count || 0} authors`}
            </span>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '0.55rem' }}>
          <span style={{
            ...
            fontSize: '0.68rem', fontWeight: 700, marginBottom: '0.45rem',
          }}>
            <FaLayerGroup size={9} />
          ...
          <Link to={`/papers/${paper.paper_id}`} style={{ textDecoration: 'none' }}>
            <p style={{
              ...
              lineHeight: 1.45, marginBottom: '0.35rem',
        ...
        <Link
          ...
          style={{
            marginTop: '0.65rem', display: 'inline-flex', alignItems: 'center', gap: 4,
