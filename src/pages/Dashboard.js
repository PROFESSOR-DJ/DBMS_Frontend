...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const papersPerYear = rawPPY.map(i => ({
        year: i._id ?? i.publish_year ?? i.year,
        count: i.count ?? i.paper_count ?? 0
      })).filter(i => i.year != null && i.count > 0).sort((a, b) => a.year - b.year);
      const rawJournals = data.analytics?.top_journals || data.top_journals || [];
      const topJournals = rawJournals.map(i => ({
        name: i.journal ?? i._id ?? i.name,
        value: i.count ?? i.paper_count ?? 0
      })).filter(i => i.name).slice(0, 5);
      const rawAuthors = data.analytics?.top_authors || data.top_authors || [];
      const topAuthors = rawAuthors.map(i => ({
        author: i.author ?? i._id ?? i.name,
        papers: i.count ?? i.paper_count ?? 0
      })).filter(i => i.author).slice(0, 10);
      const totalPapers = data.database_overview?.mysql?.total_papers ?? data.database_overview?.mongodb?.total_papers ?? data.stats?.totalPapers ?? data.total_papers ?? 0;
      const uniqueAuthors = data.database_overview?.mysql?.total_authors ?? data.stats?.uniqueAuthorCount ?? data.unique_authors ?? 0;
      const totalJournals = data.database_overview?.mongodb?.unique_journals ?? data.stats?.uniqueJournalCount ?? data.unique_journals ?? 0;
    ...
    transition: 'background 0.4s ease'
      ...
      padding: '2rem 1.5rem'
        ...
        position: 'relative'
          ...
          marginBottom: '0.5rem'
        }}>✦ Research Intelligence Platform</p>
          <h1 style={{
        ...
        {}
        {stats.papersPerYear.length > 0 && <div className="animate-fade-in" style={{
        marginBottom: '2.5rem'
      }}>
        ...
        {}
        {stats.topAuthors.length > 0 && <div className="animate-fade-in" style={{
        marginBottom: '2.5rem'
      }}>
          ...
          gap: '1.5rem'
            ...
            e.currentTarget.style.transform = 'translateY(0)';
              ...
              borderBottom: `1px solid ${t.divider}`
                ...
                {dbStats.map(([k, v]) => <div key={k} style={{
              display: 'flex',
              justifyContent: 'space-between',
              alignItems: 'center',
              paddingTop: '0.75rem'
                ...
                WebkitTextFillColor: 'transparent',
                backgroundClip: 'text'
              }}>{typeof v === 'number' ? v.toLocaleString() : v}</span>
                  </div>)}
              </div>)}
          </div>
        </div>
        ...
        gap: '1.5rem'
          ...
          <div style={glassPanel}>
            ...
            gap: '0.75rem'
              ...
              icon: Icon,
              color
            }) => <button key={href} onClick={() => window.location.href = href} style={{
              width: '100%',
