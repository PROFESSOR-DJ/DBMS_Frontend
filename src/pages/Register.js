...
const Register = () => {
  ...
  const fields = [
    { name: 'name', type: 'text', icon: FaUser, label: 'Full Name', placeholder: 'John Doe', required: true },
    { name: 'email', type: 'email', icon: FaEnvelope, label: 'Email Address', placeholder: 'you@example.com', required: true },
    { name: 'password', type: 'password', icon: FaLock, label: 'Password', placeholder: '••••••••', required: true, minLength: 6 },
    { name: 'confirmPassword', type: 'password', icon: FaLock, label: 'Confirm Password', placeholder: '••••••••', required: true, minLength: 6 },
  ];
  ...
  return (
    <div style={{
      ...
      <Orb style={{ width: 380, height: 380, background: 'radial-gradient(circle,rgba(99,102,241,0.22) 0%,transparent 70%)', top: '-80px', right: '-80px', animation: 'orbFloat 9s ease-in-out infinite' }} />
      <Orb style={{ width: 300, height: 300, background: 'radial-gradient(circle,rgba(168,85,247,0.18) 0%,transparent 70%)', bottom: '-60px', left: '-60px', animation: 'orbFloat2 11s ease-in-out infinite' }} />
      <Orb style={{ width: 180, height: 180, background: 'radial-gradient(circle,rgba(16,185,129,0.10) 0%,transparent 70%)', top: '40%', right: '15%', animation: 'orbFloat 14s ease-in-out infinite reverse' }} />
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.875rem' }}>
            Join <span style={{ color: '#818cf8' }}>ResearchDB</span> and explore research papers
          </p>
        </div>

        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {fields.map(({ name, type, icon: Icon, label, placeholder, required, minLength }) => (
            <div key={name}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {label}
              </label>
              <div style={{ position: 'relative' }}>
                <Icon size={13} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: focused[name] ? '#818cf8' : '#475569', transition: 'color 0.3s ease' }} />
                <input
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginTop: '0.25rem' }}>
            <input type="checkbox" required style={{ accentColor: '#6366f1', width: 15, height: 15 }} />
            <span style={{ fontSize: '0.83rem', color: '#64748b' }}>
              I agree to the{' '}
              <Link to="/terms" style={{ color: '#818cf8', textDecoration: 'none', fontWeight: 500 }}>Terms &amp; Conditions</Link>
            </span>
          </label>
          ...
          <button
            ...
            style={{
              ...
              transition: 'all 0.3s ease', marginTop: '0.25rem',
            }}
            onMouseEnter={e => { if (!loading) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 30px rgba(99,102,241,0.6)'; } }}
