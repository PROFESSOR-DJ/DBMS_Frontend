...
const Authors = () => {
  ...
  const fetchAuthors = async () => {
    ...
    try {
      if (searchTerm.trim()) {
        // Live search via /authors/search
  ...
  const handleDelete = async id => {
    if (window.confirm('Delete this author?')) {
      try {
        ...
        // trg_before_author_delete sends a descriptive message
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '2rem 1.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
            ...
            <button onClick={() => navigate('/authors/new')}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 7,
                       padding: '0.6rem 1.1rem', borderRadius: 11, background: t.accentGrad,
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 240, position: 'relative' }}>
            <FaSearch size={14} style={{ position: 'absolute', left: 14, top: '50%',
            ...
            <input type="text" value={searchTerm}
              ...
              style={{ width: '100%', padding: '0.8rem 1rem 0.8rem 2.6rem',
                       borderRadius: 12, background: searchFocused ? t.cardHover : t.inputBg,
                       border: `1px solid ${searchFocused ? t.accentBorder : t.inputBorder}`,
                       color: t.inputColor, fontSize: '0.9rem', outline: 'none',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value)}
              style={{ padding: '0.8rem 0.9rem', borderRadius: 12, background: t.inputBg,
        ...
        {loading ? (
          ...
          <div style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`,
            ...
            {searchTerm && (
              <button onClick={() => setSearchTerm('')}
                style={{ color: t.accentText, background: 'none', border: 'none',
          ...
          <div className="animate-fade-in"
            ...
            {/* Column headers */}
            <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 160px 120px 120px',
            ...
            {sortedAuthors.map((author, index) => {
              ...
              return (
                <div key={author.author_id || index}
                  style={{ display: 'grid', gridTemplateColumns: '60px 1fr 160px 120px 120px',
                  ...
                  <div>
                    {index < 3 ? (
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: rc.bg,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: t.accentGrad,
                                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                                  flexShrink: 0 }}>
                  ...
                  {/* Paper count + mini bar */}
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: '0.875rem', fontWeight: 700,
                                     color: t.accentText, minWidth: 32 }}>{paperCount}</span>
                      <div style={{ flex: 1, height: 5, background: t.cardBorder,
                                    borderRadius: 999, overflow: 'hidden' }}>
                        <div style={{ width: `${barWidth}%`, height: '100%',
                                      background: t.accentGrad, borderRadius: 999 }} />
                      </div>
                    </div>
                  </div>

                  {/* ID */}
                  <span style={{ fontSize: '0.78rem', color: t.textMuted, fontFamily: 'monospace' }}>
                    {author.author_id || 'N/A'}
                  </span>

                  {/* Actions */}
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => navigate(`/authors/edit/${author.author_id}`)}
                      style={{ width: 30, height: 30, borderRadius: 8, background: t.accentBg,
                               border: `1px solid ${t.accentBorder}`, color: t.accentText,
